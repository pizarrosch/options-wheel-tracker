...
function StatCard({ label, val, color, sub }) {
  return (
    <div style={cardStyle}>
      ...
      <div style={{ color: color||T, fontSize: 18, fontWeight: 700, fontFamily: 'monospace' }}>{val}</div>
      {sub && <div style={{ color: M, fontSize: 11, marginTop: 3 }}>{sub}</div>}
    </div>
  );
}
...
function GreeksCard({ delta, theta, vega }) {
  const row = (lbl, val, col) => (
    <div style={{ display:'flex', justifyContent:'space-between', padding:'7px 0', borderBottom:`1px solid ${D}` }}>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4,1fr)', gap:10 }}>
        ...
        <StatCard label="Win Rate" val={NUM(winRate,1)+'%'} color={winRate>=70?G:winRate>=50?YL:R} sub={`${wins}/${closed.length} trades`} />
            ...
            <div style={cardStyle}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:6, marginTop:4 }}>
                {st.expirations.slice(0,10).map(p => {
                  ...
                  return (
                    <div key={p.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 10px', background:B, borderRadius:6, border:`1px solid ${D}` }}>
                      <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
                        <span style={{ fontWeight:700 }}>{p.ticker}</span>
                        <span style={{ fontSize:11, background:PC[p.phase]+'22', color:PC[p.phase], padding:'2px 6px', borderRadius:4 }}>{p.phase}</span>
                        <span style={{ color:M, fontSize:12 }}>${p.strike}</span>
                      </div>
                      <div style={{ display:'flex', gap:10, alignItems:'center' }}>
                        {!isMobile && <span style={{ color:M, fontSize:12 }}>{p.expiry}</span>}
                        <span style={{ color:col, fontFamily:'monospace', fontWeight:700 }}>{p.dte<=0?'EXP':p.dte+'d'}</span>
                      </div>
                    </div>
                  );
                })}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            {isMobile ? (
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                {filtered.length===0 && <div style={{ ...cardStyle, textAlign:'center', color:M, padding:40 }}>No positions. Tap ＋ to add a trade.</div>}
                {filtered.map(pos => <PositionCard key={pos.id} pos={pos} onEdit={doEdit} onDelete={doDelete} />)}
              </div>
            ) : (
